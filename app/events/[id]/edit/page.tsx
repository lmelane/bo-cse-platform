...
export default function EditEventPage() {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        setLoadError('Impossible de charger cet evenement');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await eventsApi.update(eventId, dataToSubmit);
      ...
      <AdminLayout>
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="flex items-center gap-3">
            <Link href="/events" className="p-1.5 rounded-md hover:bg-neutral-100 transition-colors">
              <ArrowLeft className="w-4 h-4 text-neutral-600" />
            </Link>
            <h1 className="text-base font-semibold text-neutral-900">Modifier l&apos;evenement</h1>
          </div>
          <div className="bg-white border border-neutral-200 rounded-lg p-8 text-center">
            <p className="text-sm text-neutral-500">{loadError}</p>
            <Link href="/events" className="mt-4 inline-block text-sm text-neutral-600 underline">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white border border-neutral-200 rounded-lg p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Edit-only fields: status and ticket status */}
              <div>
                <label className={labelClass}>Annuler l&apos;evenement ?</label>
                <select
                  name="status"
                  ...
                  className={inputClass}
                >
                  <option value="scheduled">Actif (plannifie)</option>
                  <option value="cancelled">Annule</option>
                </select>
                <p className="text-xs text-neutral-500 mt-1">
                  Les statuts En cours et Termine sont geres automatiquement
                </p>
              </div>

              <div>
                <label className={labelClass}>Statut des reservations</label>
                <select
                  name="ticketStatus"
                ...
                >
                  ...
                  <option value="limited">Dernieres places</option>
                  <option value="sold_out">Complet</option>
                  <option value="closed">Reservations fermees</option>
                  <option value="coming_soon">Bientot disponible</option>
                </select>
                <p className="text-xs text-neutral-500 mt-1">
                  Le systeme recalcule automatiquement ce statut selon les places disponibles. Utilisez Ferme uniquement pour bloquer manuellement les reservations.
                </p>
              </div>
